...
def create_regions(world: TCSWorld) -> None:
    ...
    cantina = world.create_region(world.origin_region_name)
    ...
    goal_requires_area_completion = world.goal_area_completion_count > 0

    goal_chapter_locations_mode = world.options.goal_chapter_locations_mode.value

    chapters_to_create = world.enabled_chapters

    for episode_number in range(1, 7):
        if episode_number not in world.enabled_episodes:
            continue
        ...
        for chapter_number, chapter in enumerate(episode_chapters, start=1):
            ...
            if create_gold_bricks:
                ...
                world.gold_brick_event_count += 1
    ...
    if world.available_minikits != available_minikits_check:
    ...
    # generate_early.
    assert world.gold_brick_event_count == world._expected_gold_brick_event_count
    ...
    for purchase in starting_purchases:
        location = LegoStarWarsTCSLocation(world.player, purchase, world.location_name_to_id[purchase], cantina)
    ...
    if not world.goal_chapter:
        victory = LegoStarWarsTCSLocation(world.player, "Goal", parent=cantina)
        victory.place_locked_item(world.create_event("Victory"))
