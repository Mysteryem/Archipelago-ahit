...
def tracker_data_route(tracker: UUID) -> Response:
    ...
    if not room:
        abort(404)
    return _get_tracker_data_response(room)
...
def _get_tracker_data_response(room: Room) -> Response:
    ...
    return Response(current_app.json.dumps(result), mimetype="application/json")
